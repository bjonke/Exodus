...
{
	RECT InventoryBG = {0,0,400,400};
	HBRUSH hbr;
	hbr = CreateSolidBrush(RGB(205, 201, 201));
	FillRect(BackBuffer,&InventoryBG,hbr);
...
void CGraphics::Guild()
{
	...
	if(SActors.at(GetActiveActor(SActors)).m_Guild == cFIGHTER_GUILD)
	{
		DrawText(BackBuffer,Guilds.m_Vector.at(FighterGuildID).at(GuildTitle).c_str(),strlen(Guilds.m_Vector.at(FighterGuildID).at(GuildTitle).c_str()), &Messages, DT_CALCRECT);
	}
	else
	if(SActors.at(GetActiveActor(SActors)).m_Guild == cMAGE_GUILD)
	{
		DrawText(BackBuffer,Guilds.m_Vector.at(MageGuildID).at(GuildTitle).c_str(),strlen(Guilds.m_Vector.at(MageGuildID).at(GuildTitle).c_str()), &Messages, DT_CALCRECT);
	}
	else
	if(SActors.at(GetActiveActor(SActors)).m_Guild == cCLERIC_GUILD)
	{
		DrawText(BackBuffer,Guilds.m_Vector.at(ClericGuildID).at(GuildTitle).c_str(),strlen(Guilds.m_Vector.at(ClericGuildID).at(GuildTitle).c_str()), &Messages, DT_CALCRECT);
	}
	else
	if(SActors.at(GetActiveActor(SActors)).m_Guild == cTHIEF_GUILD)
	{
		DrawText(BackBuffer,Guilds.m_Vector.at(ThiefGuildID).at(GuildTitle).c_str(),strlen(Guilds.m_Vector.at(ThiefGuildID).at(GuildTitle).c_str()), &Messages, DT_CALCRECT);
	}

	// Choosing a class triggera
	if(PtInRect(&FighterGuildButton,Mouse))
	{
		if(KEY_DOWN(VK_LBUTTON))
		{
			GameMessageWindow.Post("FIGHTER");
			SActors.at(GetActiveActor(SActors)).m_Guild = cFIGHTER_GUILD;
		}
	}
	if(PtInRect(&MageGuildButton,Mouse))
	{
		if(KEY_DOWN(VK_LBUTTON))
		{
			GameMessageWindow.Post("MAGE");
			SActors.at(GetActiveActor(SActors)).m_Guild = cMAGE_GUILD;
		}
	}
	if(PtInRect(&ClericGuildButton,Mouse))
	{
		if(KEY_DOWN(VK_LBUTTON))
		{
			GameMessageWindow.Post("CLERIC");
			SActors.at(GetActiveActor(SActors)).m_Guild = cCLERIC_GUILD;
		}
	}
	if(PtInRect(&ThiefGuildButton,Mouse))
	{
		if(KEY_DOWN(VK_LBUTTON))
		{
			GameMessageWindow.Post("THEIF");
			SActors.at(GetActiveActor(SActors)).m_Guild = cTHIEF_GUILD;
		}
	}
